    ...
    ui(new Ui::tradingDialog)
{
    ...
    ui->PEXAvailableLabel->setPalette(sample_palette);
    ...
    CreateOrderBookTables(*ui->BidsTable,QStringList() << "TOTAL(PEX)"<< "PEX(SIZE)" << "BID(PEX)");
    CreateOrderBookTables(*ui->AsksTable,QStringList() << "ASK(PEX)"  << "PEX(SIZE)" << "TOTAL(PEX)");
    /*OrderBook Table Init*/
    ...
    ui->MarketHistoryTable->setHorizontalHeaderLabels(QStringList()<<"DATE"<<"BUY/SELL"<<"BID/ASK"<<"TOTAL UNITS(PEX)"<<"TOTAL COST(PEX");
...
void tradingDialog::UpdaterFunction(){
    //PEXst get the main exchange info in order to populate qLabels in maindialog. then get data
...
QString tradingDialog::GetMarketSummary(){
     ...
     QString Response = sendRequest("https://bittrex.com/api/v1.1/public/GetMarketSummary?market=evo-PEX");
     return Response;
}

QString tradingDialog::GetOrderBook(){

      QString  Response = sendRequest("https://bittrex.com/api/v1.1/public/getorderbook?market=PEX-PEX&type=both&depth=50");
      return Response;
}

QString tradingDialog::GetMarketHistory(){
      QString Response = sendRequest("https://bittrex.com/api/v1.1/public/getmarkethistory?market=PEX-PEX&count=100");
      return Response;
}
...
QString tradingDialog::BuyPEX(QString OrderType, double Quantity, double Rate){
    ...
    QString URL = "https://bittrex.com/api/v1.1/market/";
            ...
            URL += "?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=PEX-PEX&quantity=";
...
QString tradingDialog::SellPEX(QString OrderType, double Quantity, double Rate){
...
QString tradingDialog::GetOpenOrders(){
    QString URL = "https://bittrex.com/api/v1.1/market/getopenorders?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=PEX-PEX";
...
QString tradingDialog::GetDepositAddress(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getdepositaddress?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&currency=PEX";
...
QString tradingDialog::GetAccountHistory(){
    ...
    QString URL = "https://bittrex.com/api/v1.1/account/getorderhistory?apikey=";
            URL += this->ApiKey;
            URL += "&nonce=12345434&market=PEX-PEX&count=10";
...
int tradingDialog::SetExchangeInfoTextLabels(){
    ...
    ui->Ask->setText("<b>Ask:</b> <span style='font-weight:bold; font-size:14px; color:Red'>" + str.number(obj["Ask"].toDouble(),'i',8) + "</span> PEX");

    ui->Bid->setText("<b>Bid:</b> <span style='font-weight:bold; font-size:14px; color:Green;'>" + str.number(obj["Bid"].toDouble(),'i',8) + "</span> PEX");

    ui->volumet->setText("<b>PEX Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["Volume"].toDouble(),'i',8) + "</span> PEX");

    ui->volumeevo->setText("<b>PEX Volume:</b> <span style='font-weight:bold; font-size:14px; color:blue;'>" + str.number(obj["BaseVolume"].toDouble(),'i',8) + "</span> PEX");
...
void tradingDialog::ParseAndPopulateOrderBookTables(QString OrderBook){
    ...
    foreach (const QJsonValue & value, BuyArray)
    {
        ...
        ui->PEXSupply->setText("<b>Supply:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(PEXSupply,'i',8) + "</span><b> PEX</b>");
        ui->EvoSupply->setText("<span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(EvoSupply,'i',8) + "</span><b> PEX</b>");
        ...
        ui->PEXDemand->setText("<b>Demand:</b> <span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(PEXDemand,'i',8) + "</span><b> PEX</b>");
        ui->EvoDemand->setText("<span style='font-weight:bold; font-size:14px; color:blue'>" + str.number(EvoDemand,'i',8) + "</span><b> PEX</b>");
    ...
    switch (index){
                case 0:    //buy tab is active
                           ...
                           if(Response.size() > 0 && Response != "Error"){
                               ...
                               QString balance = GetBalance("PEX");
                ...
                case 1: //sell tab active
                                   ...
                                   if(Response.size() > 0 && Response != "Error"){
                                       ...
                                       ui->PEXAvailableLabel->setText(str.number(ResultObject["Available"].toDouble(),'i',8));
                                     }
                ...
                case 6://show balance tab
                       Response = GetBalance("PEX");
                         if(Response.size() > 0 && Response != "Error"){
                          DisplayBalance(*ui->PosexBalanceLabel,*ui->PosexAvailableLabel,*ui->PosexPendingLabel, QString::fromUtf8("PEX"),Response);
                         }

                         Response = GetBalance("PEX");

                       if(Response.size() > 0 && Response != "Error"){
                         DisplayBalance(*ui->PEXBalanceLabel,*ui->PEXAvailableLabel,*ui->PEXPendingLabel, QString::fromUtf8("PEX"),Response);
                        }
                break;
...
void tradingDialog::CalculateSellCostLabel(){
    ...
    double Quantity = ui->UnitsInputPEX->text().toDouble();
    double cost = ((price * Quantity) - ((price * Quantity / 100) * 0.25));
...
{
    //calculate amount of PEX that can be gained from selling PEX available balance
    QString responseA = GetBalance("PEX");
    QString str;
    QJsonObject ResultObject =  GetResultObjectFromJSONObject(responseA);

    double AvailablePEX = ResultObject["Available"].toDouble();

    ui->UnitsInputPEX->setText(str.number(AvailablePEX,'i',8));
}

void tradingDialog::on_Buy_Max_Amount_clicked()
{
    //calculate amount of currency than can be brought with the PEX balance available
    QString responseA = GetBalance("PEX");
    ...
    double Result = (AvailablePEX / CurrentASK);
...
void tradingDialog::on_BuyPEX_clicked()
{
    double Rate;
    double Quantity;
    ...
    reply = QMessageBox::question(this,"Buy Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            if (reply == QMessageBox::Yes) {
                ...
                QString Response =  BuyPEX(Order,Quantity,Rate);
...
void tradingDialog::on_SellPEXBTN_clicked()
{
    double Rate;
    double Quantity;

    Rate     = ui->SellBidPriceEdit->text().toDouble();
    Quantity = ui->UnitsInputPEX->text().toDouble();
    ...
    reply = QMessageBox::question(this,"Sell Order",Msg,QMessageBox::Yes|QMessageBox::No);
            ...
            QString Response =  SellPEX(Order,Quantity,Rate);
            QJsonDocument jsonResponse = QJsonDocument::fromJson(Response.toUtf8());          //get json from str.
            QJsonObject  ResponseObject = jsonResponse.object();                              //get json obj
...
void tradingDialog::on_UnitsInputPEX_textChanged(const QString &arg1)
{
     CalculateSellCostLabel(); //update cost
}
