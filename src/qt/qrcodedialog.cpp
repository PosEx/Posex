...
{
    QString ret = QString("Posex:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non PEX unit input in lnReqAmount, we generate the URI with PEX as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(PosexUnits::format(PosexUnits::PEX, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
